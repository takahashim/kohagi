...
spawn the process, write JSONL, read JSONL.

- [`rails_open3.rb`](rails_open3.rb) — the stdio protocol from Ruby/Rails, the
  pattern any language can copy.
- [`openai_proxy/`](openai_proxy/) — an OpenAI-compatible `/v1/embeddings`
  endpoint, in Python, Ruby and TypeScript, so existing OpenAI code works by
  swapping `base_url`.
